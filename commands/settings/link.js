...
module.exports = class Link extends Command {
    ...
    async handle({msg, client, settings, plugins, args}, responder) {
        ...
        const user = await client.db.prepare("SELECT * FROM links WHERE user = ?").get(msg.author.id);
        ...
        const check = await client.db.prepare("SELECT vime FROM links WHERE vime = ?").all(vimeUser.id);
        ...
        if (ans == yes) {
            await client.db.prepare("INSERT OR REPLACE INTO links (user, vime, linkTime) VALUES (?, ?, ?)").run(msg.author.id, vimeUser.id, Date.now());
    ...
    async reset ({ msg, settings, client }, responder) {
        if (!settings.vimeAccount) return responder.error("{{reset.NOT_LINKED}}");
        await client.db.prepare("UPDATE links SET vime = null WHERE user = ?").run(msg.author.id);
