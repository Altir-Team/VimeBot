...
module.exports = class Leaderboard extends Command {
    ...
    async handle ({ msg, plugins, settings, client }, responder) {
        try {
            const [leaderboardObject] = await responder.selection(plugins.get('vimeworld').leaderboards, { mapFunc: x => x.description, title: '{{MENUS.CHOOSE_BOARD}}' });
            ...
            if (!board.records?.length) return await responder.error('{{FAILED_FETCH}}');
            ...
            const list = await Promise.all(board.records.batch(itemsCount).map(async (x, bunchIndex) => {
                const embed = { title: responder.t(['user', 'guild'].includes(leaderboardObject.type) ? '{{TOP_SINGLE_TITLE}}' : '{{TOP_TITLE}}' + (sortType == '{{MENUS.SKIP}}' ? '' : ` ({{WITH_SORT}})`), { sortType, boardType: ['user', 'guild'].includes(leaderboardObject.type) ? responder.t(`{{TYPES.${leaderboardObject.type.toUpperCase()}}}`) : plugins.get('vimeworld').games.find(x => x.id == leaderboardObject.type.replace(/_monthly/g, '').toUpperCase()).name }), color: client.vimeColor };
                embed.description = x.map((o, itemIndex) => { const item = new ListItem(o); return responder.t(`**${bunchIndex * itemsCount + itemIndex + 1}.** ${item.toString()}${statsLocales[leaderboardObject.type] ? ` **${Object.entries(statsLocales[leaderboardObject.type]).map(([stat, translation]) => `${translation}: \`${item.rawObject[stat]}\``).join('. ')}**`: ''}`, { ...item, addition: item.rawObject }); }).join('\n');
                if (settings.vimeAccount) {
                    ...
                    if (board.leaderboard.type == "guild") {
                        try {
                            ...
                            if (user.guild) {
                                const index = board.records.findIndex(x => x.id == user.guild.id);
                                embed.footer.text = responder.t(index === -1 ? '{{FOOTER.GUILD_NO_PLACE}}' : '{{FOOTER.GUILD_ON_PLACE}}', { place: index + 1 });
                            }
                        } catch {}
                    } else {
                        const index = board.records.findIndex(x => x.id == settings.vimeAccount);
